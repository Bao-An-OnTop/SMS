...
def run(phone, i):
    functions = [
        ...
        med, ghn, shop, gala, fa, cathay, vina, ahamove, air, otpmu, vtpost, shine, domi, fm, cir, hoanvu, shop, beau, fu, lote, lon
    ]
...
{vang}╔══════════════════════════════════════════════════════╗
{vang}║{do}🔥 ADMIN: {xanhla}BAO AN                                                              
{vang}║{xanhla}🔥 DEVELOP: {vang}MANH DANG KHOI                                                                                      
{vang}║{xanhnhat}🔥 TOOL AUTO SPAM + CALL                                                                                                         
{vang}╚══════════════════════════════════════════════════════╝
"""
...
# Gọi hàm hiển thị banner
banner()

# Chương trình chính
if __name__ == "__main__":
    print(f"{xanhla}BAO AN ON TOP{xanhnhat}")

    while True:
        phone = input(f"{trang}Nhập số điện thoại: {xanhla}")

        if phone == '0328149520':  
            print(f"{do}Đùa TAO À ?")
            continue  # Yêu cầu nhập lại

        if phone.isdigit() and len(phone) >= 10:  # Kiểm tra số hợp lệ
            break  # Nếu hợp lệ thì thoát vòng lặp
        else:
            print(f"{vang}Số điện thoại không hợp lệ, vui lòng nhập lại!{trang}")

    count = int(input(f"{trang}Nhập số lần spam: {xanhla}"))
